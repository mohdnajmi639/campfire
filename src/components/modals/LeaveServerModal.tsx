...
export function LeaveServerModal() {
  ...
  const handleLeave = async () => {
    setIsLoading(true);
    try {
      await fetch(`/api/servers/${data.server?._id}/leave`, { method: "PATCH" });
      onClose();
      router.push("/");
      router.refresh();
    } catch (error) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 animate-fade-in">
      <div className="w-full max-w-md rounded-lg bg-discord-channel p-6 shadow-2xl animate-scale-in">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-xl font-bold text-white">Leave Server</h2>
        ...
        <p className="mb-6 text-sm text-discord-muted">
          ...
          <span className="font-semibold text-campfire-orange">{data.server?.name}</span>?
          You won&apos;t be able to rejoin unless you&apos;re re-invited.
        </p>
        ...
        <div className="flex justify-end gap-3">
          <button
            onClick={onClose}
            disabled={isLoading}
            className="rounded-sm px-4 py-2 text-sm text-discord-muted hover:text-white transition-colors"
          >
            Cancel
          </button>
          <button
